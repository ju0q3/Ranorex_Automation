...
verboseEcho "       ... Script Commence" | tee C:\\ste\\regression_test\\Create_PDS.txt

verboseEcho "           ..." | tee -a C:\\ste\\regression_test\\Create_PDS.txt
...
verboseEcho "Section Header ... Get Settings" | tee -a C:\\ste\\regression_test\\Create_PDS.txt
...
while getopts "v:t:u:s:r:p:d:m:h:" OPTION
   ...
   do
      ...
      case $OPTION in
         ...
         verboseEcho "          ${LABOPT} ... ${OPTARG}" | tee -a C:\\ste\\regression_test\\Create_PDS.txt
...
verboseEcho "Section Footer ... Get Settings" | tee -a C:\\ste\\regression_test\\Create_PDS.txt
...
verboseEcho "Section Header ... Run CreatePDS.sh" | tee -a C:\\ste\\regression_test\\Create_PDS.txt
...
ssh ${USER}@${V_HOST} bash -l -c '"cleartool' setview -login -exec ''\''createPDS.sh' -e -u ${USER} -d ${USER} '-F'\''  '${NEW_VIEW}'"'



verboseEcho "               ..." | tee -a C:\\ste\\regression_test\\Create_PDS.txt

verboseEcho "Section Footer ... Run CreatePDS.sh" | tee -a C:\\ste\\regression_test\\Create_PDS.txt
...
verboseEcho "       ... Script Complete" | tee -a C:\\ste\\regression_test\\Create_PDS.txt
