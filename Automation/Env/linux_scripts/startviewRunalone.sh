...
verboseEcho "Get Settings" | tee C:\\ste\\regression_test\\Start_Runalone.txt
...
while getopts "v:t:u:s:r:p:d:m:h:" OPTION
   ...
   do
      ...
      case $OPTION in
         ...
         verboseEcho "          ${LABOPT} ... ${OPTARG}" | tee -a C:\\ste\\regression_test\\Start_Runalone.txt
...
verboseEcho "Prepare and Start PDS for Automation" | tee -a C:\\ste\\regression_test\\Start_Runalone.txt
...
then
   ...
   verboseEcho "                   ..." | tee -a C:\\ste\\regression_test\\Start_Runalone.txt

   verboseEcho "         VIEW TYPE ... ${VTYPE}" | tee -a C:\\ste\\regression_test\\Start_Runalone.txt

   verboseEcho "      CURRENT VIEW ... ${NEW_VIEW}" | tee -a C:\\ste\\regression_test\\Start_Runalone.txt

   verboseEcho "     PREVIOUS VIEW ... ${PREV_VIEW}" | tee -a C:\\ste\\regression_test\\Start_Runalone.txt

   verboseEcho "                DB ... ${DB}" | tee -a C:\\ste\\regression_test\\Start_Runalone.txt

   verboseEcho "        RESET TDMS ... ${RESET_TDMS}" | tee -a C:\\ste\\regression_test\\Start_Runalone.txt
...
then
   ...
   verboseEcho "                   ... Starting PDS - Skip Reset TDMS" | tee -a C:\\ste\\regression_test\\Start_Runalone.txt

 
   RUN_ARGS="-nodb -ste ${STE_TAG} -run -notdms div1"
   
   verboseEcho "RUN_ARGS: ${RUN_ARGS}"
   ...
   verboseEcho "                   ... Starting PDS - Reset TDMS" | tee -a C:\\ste\\regression_test\\Start_Runalone.txt
...
ssh ${USER}@${V_HOST} bash -l -c '"/homes/smoker/preparePDSRunaloneAutomation '${RUN_ARGS}'"'
...
verboseEcho "Complete" | tee -a C:\\ste\\regression_test\\Start_Runalone.txt
