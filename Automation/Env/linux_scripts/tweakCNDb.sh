...
verboseEcho "Get Settings" | tee -a C:\\ste\\regression_test\\Tweak_Label.txt
...
while getopts "v:t:u:s:r:p:d:l:m:h:" OPTION
   ...
   do
      ...
      case $OPTION in
         ...
         l) LABOPT="    LOCATION" LOCATION="${OPTARG}";;
         ...
         verboseEcho "          ${LABOPT} ... ${OPTARG}" | tee -a C:\\ste\\regression_test\\Tweak_Label.txt
...
ssh ${USER}@${V_HOST} bash -l -c '"cleartool setview -login -exec '"'/home/smoker/tweakForCnDb'"' '${NEW_VIEW}'"'


verboseEcho "Complete" | tee -a C:\\ste\\regression_test\\Tweak_Label.txt
