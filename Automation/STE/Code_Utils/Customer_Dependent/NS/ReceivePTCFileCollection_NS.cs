...
namespace STE.Code_Utils
{
    ...
    /// </summary>
    [UserCodeCollection]
    public class ReceivePTCFileCollection_NS
    {
        ...
        [UserCodeMethod]
        public static void validateNoDC_ASBI_7ForTrain(int timeInSeconds, bool retry)
        {
        	Ranorex.Report.Info("TestStep","Validating that STE message containing added message filters has been sent within a " + timeInSeconds + "-second timespan");
        	PTC_DC_ASBI_7 ptc_dc_asbi = null;
        	ptc_dc_asbi = getMessageDC_ASBI_7(timeInSeconds, retry);
        	...
        	if (ptc_dc_asbi == null)
        	{
        		Ranorex.Report.Success("No DC-ASBI found matching filters");
        	} else {
        		Ranorex.Report.Failure("DC-ASBI message(s) found match filters");
        	}
        	return;
        }
        ...
        [UserCodeMethod]
        public static void validateDC_TCON_7_ForTrain(string filterString, int timeInSeconds=5, bool retry=true, bool validateDoesExist=true)
        {
        	Ranorex.Report.Info("TestStep","Checking for DC-TCON message.");
        	PTC_DC_TCON_7 ptc_dc_tcon = null;
            clearFilters();
            if (!string.IsNullOrEmpty(filterString) && validateDoesExist)
            {
                addValueToFilters(filterString);
                Ranorex.Report.Info("TestStep", "Checking for DC-TCON message with the following content: " + filterString);
            }
        	ptc_dc_tcon = getMessageDC_TCON_7(timeInSeconds, retry);
        	clearFilters();
            if (validateDoesExist)
        	{
        		if (ptc_dc_tcon != null)
        		{
        			Report.Success("Validation", "DC-TCON message has been received within " + timeInSeconds.ToString() + " seconds.");
        		} else {
        			Report.Failure("Failure", "No DC-TCON message has been received within " + timeInSeconds.ToString() + " seconds.");
        		}
        	} else {
        		if (ptc_dc_tcon != null)
        		{
        			Report.Failure("Failure", "DC-TCON message has been received within " + timeInSeconds.ToString() + " seconds.");
        		} else {
        			Report.Success("Validation", "No DC-TCON message has been received within " + timeInSeconds.ToString() + " seconds.");
        		}
        	}
        }
        ...
        [UserCodeMethod]
        public static void validateDC_DSSR_7(int timeInSeconds, bool retry, bool doesExist=true)
        {
        	Ranorex.Report.Info("TestStep","Validating that STE message containing added message filters has been sent within a " + timeInSeconds + "-second timespan");
        	PTC_DC_DSSR_7 ptc_dc_dssr = null;
        	ptc_dc_dssr = getMessageDC_DSSR_7(timeInSeconds, retry);
        	if (ptc_dc_dssr != null && doesExist)
        		Report.Success("Message found.");
        	else if (ptc_dc_dssr == null && doesExist)
        		Report.Failure("STE message containing filters not found");
        	else if (ptc_dc_dssr == null && !doesExist)
        		Report.Success("Message not found.");
        	else
        		Report.Failure("Message found.");
        ...
        {
            Ranorex.Report.Info("TestStep","Validating that STE message containing added message filters has not been sent within a " + timeInSeconds + "-second timespan");
            PTC_DC_TCON_7 ptc_dc_tcon = null;
            ptc_dc_tcon = getMessageDC_TCON_7(timeInSeconds, retry);
            if (ptc_dc_tcon == null)
            {
        ...
        [UserCodeMethod]
        public static void ValidateNoDG_AK01_7(int timeInSeconds, bool retry)
        {
        	Ranorex.Report.Info("TestStep","Validating that STE message containing added message filters has not been sent within a " + timeInSeconds + "-second timespan");
        	PTC_DG_AK01_7 ptc_dg_ak01 = null;
        	ptc_dg_ak01 = getMessageDG_AK01_7(timeInSeconds, retry);
         ...
         [UserCodeMethod]
        public static void ValidateNoDC_AK01_7(int timeInSeconds, bool retry)
        {
        	Ranorex.Report.Info("TestStep","Validating that STE message containing added message filters has not been sent within a " + timeInSeconds + "-second timespan");
        	PTC_DC_AK01_7 ptc_dc_ak01 = null;
        	ptc_dc_ak01 = getMessageDC_AK01_7(timeInSeconds, retry);
...
//        [UserCodeMethod] not doing ste generated message right now
//        public static bool getMessageGD_AK01_7(int timeInSeconds=5, bool retry=true)
//        {
//            bool exists = messages.SteMessageFileReader.getMessageGD_AK01_7(msgFilters.ToArray(), timeInSeconds, retry);
//            return exists;
//        }
//        
//        [UserCodeMethod]
...
//        {
//        	Ranorex.Report.Info("TestStep","Validating that STE message containing added message filters has been sent within a " + timeInSeconds + "-second timespan");
//        	bool exists = getMessageGD_AK01_7(timeInSeconds, retry);
//        	try {
//        		Validate.IsTrue(exists);
//        	} catch(RanorexException) {
//        		Report.Error("STE message containing filters not found");
...
//        
        ...
        [UserCodeMethod]
		public static void validateErrorMessage_NoMessageField(string errorText, string reason, int timeInSeconds, bool retry, bool validateDoesExist=true)
		{
			Ranorex.Report.Info("TestStep", "Searching for PTCMR error messages.");
        	bool errorResult = false;
        	
        	string[] filters = {"<ERROR_TEXT>" + errorText +"</ERROR_TEXT>", "<REASON>" + reason + "</REASON>"};
        	errorResult = messages.SteMessageFileReader.getSpecificMessage(filters, timeInSeconds, retry);
        	if (validateDoesExist)
        	{
        		if (errorResult)
        		{
        ...
        /// <summary>
        /// Validate if there is MQ_ERROR message
        /// </summary>
        /// <param name="timeInSeconds">Integer parameter</param>
        /// <param name="retry">Boolean parameter</param>
        [UserCodeMethod]
        public static void validateMQ_ERROR(int timeInSeconds, bool retry)
        {
        	Ranorex.Report.Info("TestStep","Validating that STE message containing added message filters has been sent within a " + timeInSeconds + "-second timespan");
        	PTC_MQ_ERROR_1 ptc_mq_error = null;
        ...
        /// <param name="retry">Boolean parameter</param>
        [UserCodeMethod]
        public static void validateNoMQ_ERROR(int timeInSeconds, bool retry)
        {
        	Ranorex.Report.Info("TestStep","Validating that STE message containing added message filters has been sent within a " + timeInSeconds + "-second timespan");
        	PTC_MQ_ERROR_1 ptc_mq_error = null;
        	ptc_mq_error = getMessageDMQ_ERROR(timeInSeconds, retry);
        ...
        public static void validateNoDG_TAUT_7(int timeInSeconds, bool retry)
        {
        	...
        	return;
                ...
                /// <summary>
        /// Validate if there is not DG-UDIE message sent
        /// </summary>
        /// <param name="timeInSeconds">Integer parameter</param>
        /// <param name="retry">Boolean parameter</param>
        [UserCodeMethod]
        public static void validateNoDG_UDIE_7(int timeInSeconds, bool retry)
        {
        	Ranorex.Report.Info("TestStep","Validating that STE message containing added message filters has been sent within a " + timeInSeconds + "-second timespan");
        	PTC_DG_UDIE_7 ptc_dg_udie = null;
        	ptc_dg_udie = getMessageDG_UDIE_7(timeInSeconds, retry);
        	if(ptc_dg_udie==null){
        		Ranorex.Report.Success("No DG-UDIE message found");
        	}
        	else{
        		Ranorex.Report.Failure("DG-UDIE message found");
        	}
        	return;
        }
    }
